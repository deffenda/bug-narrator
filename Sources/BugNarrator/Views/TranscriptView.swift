...
struct TranscriptView: View {
    ...
    private func exportReviewItemCard(_ item: IssueExportReviewItem) -> some View {
    ...
    private func exportReviewMatchRow(_ match: SimilarIssueMatch, issueID: UUID, isSelected: Bool) -> some View {
        Button {
    ...
    private func currentExportReviewItem(for issueID: UUID) -> IssueExportReviewItem? {
        appState.pendingExportReview?.items.first(where: { $0.issue.id == issueID })
    }

    private func canConfirmExportReview(_ review: IssueExportReview) -> Bool {
